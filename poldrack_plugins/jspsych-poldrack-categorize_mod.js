...
jsPsych.plugins["poldrack-categorize"] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    trial.prompt = (typeof trial.prompt === 'undefined') ? '' : trial.prompt;
		// NEW
    trial.prefeedback_text = (typeof trial.prefeedback_text === 'undefined') ? "<p class='prefeedback'></p>" : trial.prefeedback_text;
		// ---
    ...
    trial.timing_response = trial.timing_response || -1; // default is no max response time
		// NEW
    trial.timing_prefeedback_duration = trial.timing_prefeedback_duration || 0;
		// ---
    ...
    var after_response = function(info) {
      ...
      if (trial.response_ends_trial || info.rt == -1) {
				...
				setTimeoutHandlers.push(setTimeout(function() {
					display_element.append(trial.prefeedback_text);
				}, trial.timing_stim - info.rt ));

        setTimeout(function() {
          display_element.html('');
          doFeedback(correct, timeout);
        }, trial.timing_response + trial.timing_prefeedback_duration - info.rt);
